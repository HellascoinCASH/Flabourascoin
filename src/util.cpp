...
const char * const BITCOIN_CONF_FILENAME = "flabourascoin.conf";
const char * const BITCOIN_PID_FILENAME = "flabourascoind.pid";
...
#else
    const char* pszModule = "flabourascoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FlabourasCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FlabourasCoin";
#else
    // Unix
    return pathRet / ".flabourascoin";
#endif
#endif
}
